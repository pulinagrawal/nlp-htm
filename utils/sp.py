...
from htm.bindings.math import ( GetNTAReal,
...
class Columns:
  def __init__(self):
    self.array = numpy.zeros((0,), dtype=realDType)

  def resize(self, *shape):
    '''Resize the array to the new shape, filling new elements with zeros'''
    numColumns, numInputs = shape
    self.array = [numpy.zeros((numInputs,), dtype=realDType) for _ in range(numColumns)]

  def replace(self, columnIndex, vector):
    self.array[columnIndex] = vector

  def update(self, columnIndex, vector):
    self.replace(columnIndex, vector)

  def __getitem__(self, columnIndex):
    return self.array[columnIndex]

  def rightVecSumAtNZ(self, inputVector, overlaps):
    ''' for each column index i, sum inputVector[j] for all j where overlaps[i][j] is non-zero'''
    for i, column in enumerate(self.array):
      overlaps[i] = np.sum(inputVector[column])
    return overlaps
  
  def rightVecSumAtNZ_fast(self, x, y):
    return self.rightVecSumAtNZ(x, y)


class CorticalColumns(Columns):
  ...
  values
  """
  def __init__(self, numColumns, numInputs):
    self.array = [numpy.zeros((numInputs,), dtype=realDType) for _ in range(numColumns)]



class BinaryCorticalColumns(Columns):
  ...
  def __init__(self, numInputs):
    self.array = numpy.zeros(numInputs, dtype=uintType)
...
class SpatialPooler:
  ...
  def __init__(self,
    ...
    numColumns = int(columnDimensions.prod())
    ...
    numInputs = int(inputDimensions.prod())
  ...
  def _mapColumn(self, index):
    ...
    * If the topology is two dimensional, with column dimensions [3, 5] and
    ...
    inputIndex = numpy.ravel_multi_index(tuple(inputCoords), tuple(self._inputDimensions))
    return inputIndex
  ...
  def _mapPotential(self, index):
    ...
    * If the topology is two dimensional, the input space is again divided up
    ...
    selectedInputs[:] = numpy.random.choice(columnInputs, size=numPotential, replace=False)
...
class SpatialPoolerShim(SpatialPooler):

  def compute(self, inputVector, learn, activeArray):
    _inputVector = numpy.array(inputVector.dense)
    _activeArray = numpy.array(activeArray.dense)
    super().compute(_inputVector, learn, _activeArray)
    activeArray.dense = _activeArray

if __name__ == '__main__':
  sp = SpatialPooler()
  sp.printParameters()
  print("SpatialPooler test passed")